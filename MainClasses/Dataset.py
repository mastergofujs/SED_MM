...
class Dataset:
    ...
    def __load_data__(self, feature='mfccs', fold=0):
        ...
        val_split_path = '../aed_data/tut_data/train_val_split/'
        val_split_list = sorted(os.listdir(val_split_path))
        train_datas_list, val_datas_list, test_datas_list = [], [], []

        for line in open(val_split_path + val_split_list[fold - 1]):
            line = line.rstrip('\n')
            if line.split(':')[0] == 'train':
                train_datas_list = sorted(line.split(':')[1].split(','))
            else:
                val_datas_list = sorted(line.split(':')[1].split(','))
    ...
    #            'nsamples' means the number of total samples
    #            'inUnbalanced' means if load the unbalanced data, which also can be construct by mix_data().
    def __load_data2__(self, nevents, nsamples, fold, isUnbalanced=False):
        if not isUnbalanced:
            self.data_path = '../aed_data/freesound/mfccs/datas/mfccs_' + str(nevents) + '_' + str(nsamples) + '.pkl'
            self.label_path = '../aed_data/freesound/mfccs/labels/labels_' + str(nevents) + '_' + str(
                nsamples) + '.pkl'
        else:
            self.data_path = '../aed_data/freesound/mfccs/datas/DA_mfccs_' + str(nevents) + '_' + str(
                nsamples) + '.pkl'
            self.label_path = '../aed_data/freesound/mfccs/labels/DA_labels_' + str(nevents) + '_' + str(
                nsamples) + '.pkl'
        ...
        if not isUnbalanced:
            ...
            train_datas = datas[:int(nsamples * 85)]
            train_labels = labels[:int(nsamples * 85)]
        ...
        # train_datas = self.normalize(train)
        train_datas = train
        train_labels = label
    ...
    def mix_data(self, nevents, nsamples=1000, isUnbalanced=False):
        sr = self.sr
        ...
        for i in range(nsamples):
            ...
            k = self.SNR2K(mixed_data, n_data[:len(mixed_data)], dB)
        ...
        if isUnbalanced:
            with open('../aed_data/freesound/mfccs/datas/DA_mfccs_' + str(nevents) + '_' + str(
                    nsamples) + '.pkl', 'wb') as f:
                pkl.dump(datas, f)
            with open('../aed_data/freesound/mfccs/labels/DA_labels_' + str(nevents) + '_' + str(
                    nsamples) + '.pkl', 'wb') as f:
                pkl.dump(labels, f)

        else:
            with open('../aed_data/freesound/mfccs/datas/mfccs_' + str(nevents) + '_' + str(
                    nsamples) + '.pkl', 'wb') as f:
                pkl.dump(datas, f)
            with open('../aed_data/freesound/mfccs/labels/labels_' + str(nevents) + '_' + str(
                    nsamples) + '.pkl', 'wb') as f:
                pkl.dump(labels, f)
        print()
    ...
    def load_data(self, fold, nevents=0, nsamples=2000, isUnbalanced=False):
        if self.dataset == 'tut_data':
            ...
            train_datas, train_labels, test_datas, test_labels \
                = self.__load_data2__(nevents, nsamples, isUnbalanced=isUnbalanced, fold=fold)
            return train_datas, train_labels, test_datas, test_labels
        else:
